...
//---------------------------------------------------------------------------
// read_row_callback
static void PNG_read_row_callback(png_structp png_ptr, png_uint_32 row,
                                  int pass) {}
//---------------------------------------------------------------------------
// read_chunk_callback
static int PNG_read_chunk_callback(png_structp png_ptr,
                                   png_unknown_chunkp chunk) {
    // handle vpAg chunk (this will contain the virtual page size of
    // the image) vpAg chunk can be embeded by ImageMagick -trim
    // option etc. we don't care about how the chunk bit properties
    // are being provided.
    if((chunk->name[0] == 0x76 /*'v'*/ || chunk->name[0] == 0x56 /*'V'*/) &&
       (chunk->name[1] == 0x70 /*'p'*/ || chunk->name[1] == 0x50 /*'P'*/) &&
       (chunk->name[2] == 0x61 /*'a'*/ || chunk->name[2] == 0x41 /*'A'*/) &&
       (chunk->name[3] == 0x67 /*'g'*/ || chunk->name[3] == 0x47 /*'G'*/) &&
       chunk->size >= 9) {
        PNG_read_chunk_callback_user_struct *user_struct =
            reinterpret_cast<PNG_read_chunk_callback_user_struct *>(
...
#define PNG_read_be32(a)                                                       \
    (((tjs_uint32)(a)[0] << 24) + ((tjs_uint32)(a)[1] << 16) +                 \
     ((tjs_uint32)(a)[2] << 8) + ((tjs_uint32)(a)[3]))
        ...
        user_struct->metainfopushcallback(
            user_struct->callbackdata, PNG_tag_vpag_w, ttstr((tjs_int)width));
        user_struct->metainfopushcallback(
            user_struct->callbackdata, PNG_tag_vpag_h, ttstr((tjs_int)height));
        switch(unit) {
            case PNG_OFFSET_PIXEL:
                user_struct->metainfopushcallback(user_struct->callbackdata,
                                                  PNG_tag_vpag_unit,
                                                  PNG_tag_pixel);
                break;
...
void TVPLoadPNG(void *formatdata, void *callbackdata,
    ...
    try {
        ...
        png_ptr = png_create_read_struct_2(
            ...
            (png_voidp) nullptr, (png_malloc_ptr)PNG_malloc,
            (png_free_ptr)PNG_free);
        // png_ptr = png_create_read_struct( PNG_LIBPNG_VER_STRING,
        // (png_voidp)nullptr, (png_error_ptr)PNG_error,
        // (png_error_ptr)PNG_warning );
        if(!png_ptr)
            TVPThrowExceptionMessage(TVPPNGLoadError,
                                     (const tjs_char *)TVPLibpngError);
        ...
        read_chunk_callback_user_struct.metainfopushcallback =
            metainfopushcallback;
        png_set_read_user_chunk_fn(
            png_ptr, reinterpret_cast<void *>(&read_chunk_callback_user_struct),
        ...
        png_set_read_status_fn(png_ptr,
                               (png_read_status_ptr)PNG_read_row_callback);
        ...
        if(metainfopushcallback &&
           png_get_oFFs(png_ptr, info_ptr, &offset_x, &offset_y,
            ...
            metainfopushcallback(callbackdata, PNG_tag_offs_x,
                                 ttstr((tjs_int)offset_x));
            metainfopushcallback(callbackdata, PNG_tag_offs_y,
                                 ttstr((tjs_int)offset_y));
            switch(offset_unit_type) {
                case PNG_OFFSET_PIXEL:
                    metainfopushcallback(callbackdata, PNG_tag_offs_unit,
                                         PNG_tag_pixel);
                    break;
                case PNG_OFFSET_MICROMETER:
                    metainfopushcallback(callbackdata, PNG_tag_offs_unit,
        ...
        if(metainfopushcallback &&
           png_get_pHYs(png_ptr, info_ptr, &reso_x, &reso_y, &reso_unit_type)) {
            // push offset information into metainfo data
            metainfopushcallback(callbackdata, PNG_tag_reso_x,
                                 ttstr((tjs_int)reso_x));
            metainfopushcallback(callbackdata, PNG_tag_reso_y,
                                 ttstr((tjs_int)reso_y));
            switch(reso_unit_type) {
                case PNG_RESOLUTION_METER:
                    metainfopushcallback(callbackdata, PNG_tag_reso_unit,
                                         PNG_tag_meter);
                    break;
                default:
                    metainfopushcallback(callbackdata, PNG_tag_reso_unit,
        ...
        if(mode == glmPalettized) {
            ...
            // glmNormal
            // convert the image to full color ( 32bits ) one if
            // needed
        ...
        if(width >= 65536 || height >= 65536) {
            ...
            TVPThrowExceptionMessage(TVPPNGLoadError,
                                     (const tjs_char *)TVPTooLargeImage);
        }
        ...
        if(png_get_interlace_type(png_ptr, info_ptr) == PNG_INTERLACE_NONE) {
            ...
            if(!do_convert_rgb_gray) {
                ...
                for(i = 0; i < height; i++) {
                    ...
                    if(!scanline)
                        break;
                    png_read_row(png_ptr, (png_bytep)image, nullptr);
 ...
 * @param size : データサイズ
 */
static void PNG_write_write(png_structp png_ptr, png_bytep buf,
                            png_size_t size) {
 ...
 */
void TVPSaveAsPNG(void *formatdata, tTJSBinaryStream *dst,
...
void TVPLoadHeaderPNG(void *formatdata, tTJSBinaryStream *src,
                      iTJSDispatch2 **dic) {
    png_structp png_ptr = nullptr;
    png_infop info_ptr = nullptr;
    png_infop end_info = nullptr;
    try {
        ...
        *dic = TJSCreateDictionaryObject();
        ...
        png_int_32 offset_x, offset_y;
        int offset_unit_type;
        if(png_get_oFFs(png_ptr, info_ptr, &offset_x, &offset_y,
                        &offset_unit_type)) {
            val = tTJSVariant(offset_x);
            (*dic)->PropSet(TJS_MEMBERENSURE, TJS_W("offset x"), nullptr, &val,
                            (*dic));
            val = tTJSVariant(offset_y);
            (*dic)->PropSet(TJS_MEMBERENSURE, TJS_W("offset y"), nullptr, &val,
                            (*dic));
            switch(offset_unit_type) {
                case PNG_OFFSET_PIXEL:
