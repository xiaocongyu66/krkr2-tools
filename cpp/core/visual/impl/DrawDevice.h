...
public:
    ...
    //	virtual void SetTargetWindow(int wnd, bool
    // is_main) = 0;
    ...
    //! プロパティによる拡大率や、Window.layerLeft や Window.layerTop
    //! が加味された 				矩形である。
    ...
    //! @note
    //! 座標はプライマリレイヤ上の座標なので、必要ならば変換を行う
    virtual void SetCursorPos(iTVPLayerManager *manager, tjs_int x,
                              tjs_int y) = 0;
    ...
    //! @return
    //! フォーカスのあるレイヤ(nullptr=フォーカスのあるレイヤがない場合)
    ...
    //! @param		layer
    //! フォーカスのあるレイヤ(nullptr=フォーカスのあるレイヤがない状態にしたい場合)
    //! @note		Window.focusedLayer
    ...
    //! 呼ばれるが、VSync 待ちが有効になっている場合は Update
    //! 直後ではなく、
    ...
    //! @param		in_vblank
    //! 待たなくてもVBlank内だったかどうかを返す( !0 : 内、0: 外 )
    //! @param		delayed
...
public:
    ...
    //! @param		y		Y位置
    //! @return
    //! 変換に成功すれば真。さもなければ偽。PrimaryLayerManagerIndexに該当する
    //!				レイヤマネージャがなければ偽が返る
    //! @note		x, y は DestRectの (0,0)
    //! を原点とする座標として渡されると見なす
