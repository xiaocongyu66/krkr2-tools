...
BREAKPOINT_NAME = "krkr2_lldb_local_transform_sample"
...
def run_python_driver(wasm_path: Path, spec_dir: Path, output: Path) -> int:
    ...
    for spec in load_specs(spec_dir):
        ...
        write_doubles_at(base, affine_in_ptr, spec["affine_in"])
...
def read_sample(frame) -> dict:
    return {
        ...
        "result": [register_double_arg(frame, i) for i in range(6)],
    }
