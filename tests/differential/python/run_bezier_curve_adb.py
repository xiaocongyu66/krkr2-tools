#!/usr/bin/env python3
"""ADB oracle runner for bezier_curve (sub_69A754).
...
from python.run_bezier_curve_wasmtime import load_specs  # noqa: E402
from oracle_runner.adb_engine import AdbHarnessEngine  # noqa: E402
from oracle_runner.adapters import bezier_curve as adapter  # noqa: E402
...
def main() -> int:
    ...
    with AdbHarnessEngine(serial=args.serial, remote_dir=args.remote_dir) as engine:
        if use_tracer:
            tracer = FridaTracerEngine(engine, BEZIER_CURVE_TARGETS)
            tracer.attach()
        try:
            for spec in specs:
