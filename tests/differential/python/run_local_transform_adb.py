...
def main() -> int:
    ...
    LOCAL_TRANSFORM_TARGETS = None
    trace_diff = None
    if use_tracer:
        from oracle_runner.frida_tracer import FridaTracerEngine as _FT
        from oracle_runner.trace_targets import LOCAL_TRANSFORM_TARGETS as _TGT
        from oracle_runner import trace_diff as _TD
        FridaTracerEngine = _FT
        LOCAL_TRANSFORM_TARGETS = _TGT
        trace_diff = _TD
    ...
    with AdbHarnessEngine(serial=args.serial, remote_dir=args.remote_dir) as engine:
        if use_tracer:
            tracer = FridaTracerEngine(engine, LOCAL_TRANSFORM_TARGETS)
            tracer.attach()
        try:
            for spec in specs:
